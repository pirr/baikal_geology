...
def get_groups(dist_matrix, add_to_index_1=0, add_to_index_2=0):
    ...
    true_indexes[:, 0] = true_indexes[:, 0] + add_to_index_1
    true_indexes[:, 1] = true_indexes[:, 1] + add_to_index_2
...
if __name__ == '__main__':
    ...
    for gr in min_weight_max_cliques_1[0]:
        groups_1 = objs_1.loc[gr, 'group'].dropna()
        groups_2 = objs_2.loc[gr, 'group'].dropna()
        conc_groups_val = list(groups_1.values) + list(groups_2.values)
        conc_groups_val = [g for g in conc_groups_val if not pd.isnull(g)]
        gr_num = group_num + 1
        ...
        objs_1.loc[objs_1['group'].isin(groups_1), 'group'] = gr_num
        objs_2.loc[objs_2['group'].isin(groups_2), 'group'] = gr_num
